...
import { updateLeadStatus, type ActionState } from "@/app/app-shell/leads/actions";
import { LEAD_STATUS_FLOW, LEAD_STATUS_META } from "@/components/leads/lead-status";
import type { LeadStatus } from "@/lib/database-types";

const initialState: ActionState = { status: "idle" };

/**
 * Per-lead status control. Statuses are offered in canonical flow order;
 * transitions are intentionally NOT restricted (corrections must stay
 * possible). Submits to the `updateLeadStatus` server action (session
 * client + RLS); the list refreshes via revalidatePath.
 */
  ...
  return (
    <form action={formAction} className="mt-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <label htmlFor={`status-${leadId}`} className="text-xs font-medium text-slate-500">
          Status:
        </label>
        <select
          id={`status-${leadId}`}
